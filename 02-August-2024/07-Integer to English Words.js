...
var numberToWords = function (num) {
    ...
    while (num.length !== 0) {
        //five digits
        let number = '';
        let slicedOperationsPerformed = false;
        while (sliceFront && num.length > 3) {
            number += num[0];
            num.splice(0, 1);
            slicedOperationsPerformed = true;
        }
        if (slicedOperationsPerformed && sliceFront) {
            //slice front here and assign sentence with the first sentence
            sentence += `${unitsWordsMap[1][number]} Thousand`;
        }
        sliceFront = false;
        let numLength = num.length;
        let unitToRetrieve = num.shift();
        let result = unitsWordsMap[numLength][unitToRetrieve];
        sentence += ' ' + result;
    }
